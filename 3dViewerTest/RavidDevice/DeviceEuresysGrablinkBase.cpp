...
#include "DeviceEuresysGrablinkBase.h"
...
IMPLEMENT_DYNAMIC(CDeviceEuresysGrablinkBase, CDeviceFrameGrabber)

BEGIN_MESSAGE_MAP(CDeviceEuresysGrablinkBase, CDeviceFrameGrabber)
	ON_WM_TIMER()
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
static LPCTSTR g_lpszEuresysBaseTopology[EDeviceGrablinkTopology_Count] =
{
	_T("MONO"),
	_T("MONO_EF1"),
	_T("MONO_EXT1"),
...
static LPCTSTR g_lpszEuresysBaseConnector[EDeviceEuresysBaseConnector_Count] =
{
	_T("M"),
};


CDeviceEuresysGrablinkBase::CDeviceEuresysGrablinkBase()
{
...
EDeviceInitializeResult CDeviceEuresysGrablinkBase::Initialize()
{
	...
	do
	{
		...
		for(int i = 0; i < m_nConnectBoard; i++)
		{
			...
			if(nDevType == MC_BoardType_GRABLINK_BASE)
			{
		...
		if(McSetParamStr(MC_BOARD + m_nDriveIndex, MC_BoardTopology, CStringA(g_lpszEuresysBaseTopology[eTopology])) != MC_OK)
		{
		...
		EDeviceEuresysBaseConnector eConnector = EDeviceEuresysBaseConnector_Count;
		...
		if(McSetParamStr(m_hDevice, MC_Connector, CStringA(g_lpszEuresysBaseConnector[eConnector])) != MC_OK)
		{
		...
		if(McRegisterCallback(m_hDevice, CDeviceEuresysGrablinkBase::CallbackFunction, this) != MC_OK)
		{
		...
		int nSurfaceCount = 2;
		if(!GetSurfaceCount(&nSurfaceCount))
			McSetParamInt(m_hDevice, MC_SurfaceCount, nSurfaceCount);

		if(eInitType == EDeviceMulticamInitializeType_Camfile)
		{
			bool bCamMatch = false;
			if(!GetCamfileMatch(&bCamMatch))
			{
				if(bCamMatch)
					SetUpdateParameter();
			}			
...
EDeviceTerminateResult CDeviceEuresysGrablinkBase::Terminate()
{
	return __super::Terminate();
}

bool CDeviceEuresysGrablinkBase::LoadSettings()
{
	bool bReturn = false;
	...
	do
	{
		...
		AddParameterFieldConfigurations(EDeviceParameterEuresysGrablinkBase_Topology, g_lpszParamGrablinkBase[EDeviceParameterEuresysGrablinkBase_Topology - EDeviceParameterMulticam_Count], _T("0"), EParameterFieldType_Combo, ConvertStringArrayToComboElement(g_lpszEuresysBaseTopology, EDeviceEuresysBaseTopology_Count), nullptr, 0);
...
EEuresysGetFunction CDeviceEuresysGrablinkBase::GetTopology(EDeviceEuresysBaseTopology * pParam)
{
	...
	do
	{
		...
		*pParam = (EDeviceEuresysBaseTopology)_ttoi(GetParamValue(EDeviceParameterEuresysGrablinkBase_Topology));
...
EEuresysSetFunction CDeviceEuresysGrablinkBase::SetTopology(EDeviceEuresysBaseTopology eParam)
{
	...
	do
	{
		if(eParam < 0 || eParam >= EDeviceEuresysBaseTopology_Count)
		{
...
EEuresysGetFunction CDeviceEuresysGrablinkBase::GetConnector(EDeviceEuresysBaseConnector * pParam)
{
	EEuresysGetFunction eReturn = EEuresysGetFunction_UnknownError;
	...
	do
	{
		...
		*pParam = (EDeviceEuresysBaseConnector)_ttoi(GetParamValue(EDeviceParameterEuresysGrablinkBase_Connector));
...
EEuresysSetFunction CDeviceEuresysGrablinkBase::SetConnector(EDeviceEuresysBaseConnector eParam)
{
	EEuresysSetFunction eReturn = EEuresysSetFunction_UnknownError;
	...
	do
	{
		if(eParam < 0 || eParam >= EDeviceEuresysBaseConnector_Count)
		{
			eReturn = EEuresysSetFunction_NotFindCommandError;
			break;
		}
	...
	strMessage.Format(CMultiLanguageManager::GetString(ELanguageParameter__s_tochangetheparameter_s_from_s_to_s), !eReturn ? CMultiLanguageManager::GetString(ELanguageParameter_succeed) : CMultiLanguageManager::GetString(ELanguageParameter_failed), g_lpszParamGrablinkBase[eSaveID - EDeviceParameterMulticam_Count], g_lpszEuresysBaseConnector[nPreValue], g_lpszEuresysBaseConnector[eParam]);
...
bool CDeviceEuresysGrablinkBase::OnParameterChanged(int nParam, CString strValue)
{
	bool bReturn = false;
	...
	do
	{
		...
		case EDeviceParameterEuresysGrablinkBase_Topology:
			bReturn = !SetTopology((EDeviceEuresysBaseTopology)_ttoi(strValue));
			break;
		case EDeviceParameterEuresysGrablinkBase_Connector:
			bReturn = !SetConnector((EDeviceEuresysBaseConnector)_ttoi(strValue));
			break;
		default:
			bFoundID = false;
			break;
